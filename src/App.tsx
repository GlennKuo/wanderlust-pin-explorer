...
function BannerCarousel() {
  const images = useMemo(() => [{
    url: "https://images.pexels.com/photos/2082103/pexels-photo-2082103.jpeg",
    title: "Paris, France"
  }, {
    url: "https://images.pexels.com/photos/1822605/pexels-photo-1822605.jpeg",
    title: "Tokyo, Japan"
  }, {
    url: "https://images.pexels.com/photos/32715940/pexels-photo-32715940.jpeg",
    title: "New York, USA"
  }, {
    url: "https://images.pexels.com/photos/31726431/pexels-photo-31726431.jpeg",
  ...
  return <div className="relative h-[100vh] min-h-[220px] w-full overflow-hidden">
      {/* 疊放全部圖片，用透明度切換避免布局跳動 */}
      {images.map((img, i) => <img key={img.url} src={img.url} alt={img.title} className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-700 ${i === idx ? "opacity-100" : "opacity-0"}`} loading={i === 0 ? "eager" : "lazy"} />)}
      ...
      <div className="absolute left-0 top-16 px-4 pt-6">
        <div className="mx-auto max-w-6xl text-white">
          <h1 className="text-3xl font-semibold tracking-tight md:text-6xl">Plan Your Journey</h1>
...
export default function App() {
  ...
  const handleAddPin = useCallback((pos: LatLngLiteral, name: string) => {
    setPins(prev => prev.length >= 3 ? prev : [...prev, {
      id: uid(),
      position: pos,
      name
    }]);
  ...
  return <div className="min-h-screen w-full bg-gradient-to-b from-[#eaf3ff] via-[#f7fbff] to-[#e6f1ff] text-slate-800">
      <Navigation />
      <div className="pt-16">
        <BannerCarousel />
      </div>
      ...
      <section id="destinations" className="mx-auto max-w-5xl px-4 pt-8 md:pt-12">
        ...
        {/* chips overlay */}
          <div className="pointer-events-none absolute left-3 bottom-3 flex flex-wrap gap-2">
            {pins.map(p => <div key={p.id} className={`pointer-events-auto inline-flex items-center gap-2 rounded-full border backdrop-blur px-3 py-1 shadow cursor-pointer transition-colors ${selectedPin === p.id ? 'bg-blue-100/90 border-blue-300' : 'bg-white/90 hover:bg-slate-50/90'}`} onClick={() => setSelectedPin(p.id)}>
                <span className={selectedPin === p.id ? "text-blue-500" : "text-rose-500"}>📍</span>
                <span className="text-sm font-medium">{p.name}</span>
                <button onClick={e => {
              e.stopPropagation();
              handleRemovePin(p.id);
            }} className="ml-1 text-slate-500 hover:text-rose-600">×</button>
              </div>)}
          </div>
        </div>
      </section>
        ...
        <div className="rounded-3xl border bg-white/90 backdrop-blur shadow-xl p-6 md:p-8">
          ...
          <div className="mt-6 space-y-4">
            <div className="space-y-1">
              ...
              <input type="text" placeholder="Enter up to 3 destinations (comma-separated)" className="w-full rounded-2xl border px-4 py-3 bg-slate-50" value={location} onChange={e => handleLocationChange(e.target.value)} />
